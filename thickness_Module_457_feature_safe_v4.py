...
def _build_bulk_valid_mask(t_list, y_list, tol_um, p2p_thresh_um):
    """Build a boolean mask for the bulk material points.

    Strategy: histogram / mode-based bulk extraction.
    The bulk material is by far the densest cluster of thickness values in
    the profile. Anything that lives far from that cluster - groove walls,
    hole edges, sensor dropouts - falls outside the bulk band and is masked
    out, regardless of WHERE in time it appears or how many separate
    features exist.

    Adaptive scale:
    - Histogram bin width is derived from a robust noise estimate (MAD) of
      the data itself; it is NOT a hard-coded "1 micron" assumption.
    - The bulk band half-width is k*sigma_peak, measured on the peak
      samples ONLY, so feature walls do not contaminate the noise estimate.

    Returns (valid_mask, meta). On any failure to find a clean peak the
    function falls back to "all valid" and records the reason in meta.
    """
    n = len(y_list)
    meta = {
        "feature_count": 0,
        "feature_removed_samples": 0,
        "feature_threshold_um": None,
        "feature_rejection_used": False,
        "feature_rejection_reason": "not_needed",
    }
    if n < 8:
        return [True] * n, meta

    y_mm = np.asarray(y_list, dtype=float)

    # Rough noise scale from the (possibly contaminated) full profile.
    # Used only to size the histogram bins - order of magnitude is enough.
    med_mm = float(np.median(y_mm))
    mad_mm = float(np.median(np.abs(y_mm - med_mm)))
    robust_sigma_mm = 1.4826 * mad_mm

    # Adaptive bin width: ~half a robust sigma per bin gives a sharp peak
    # without splintering it. Floor avoids thousands of bins on very quiet
    # data; ceiling keeps the peak resolvable vs feature walls.
    bin_width_mm = max(0.0005, min(0.005, 0.5 * robust_sigma_mm))

    y_min = float(np.min(y_mm))
    y_max = float(np.max(y_mm))
    span_mm = y_max - y_min
    if span_mm <= bin_width_mm:
        meta["feature_rejection_reason"] = "single_bin_profile"
        return [True] * n, meta

    n_bins = int(np.ceil(span_mm / bin_width_mm)) + 1
    if n_bins < 5:
        meta["feature_rejection_reason"] = "too_few_bins"
        return [True] * n, meta

    edges = y_min + bin_width_mm * np.arange(n_bins + 1)
    counts, _ = np.histogram(y_mm, bins=edges)

    peak_idx = int(np.argmax(counts))
    peak_count = int(counts[peak_idx])
    if peak_count < 5:
        meta["feature_rejection_reason"] = "peak_too_weak"
        return [True] * n, meta

    # FWHM: expand outward from the peak while counts stay above half-peak.
    half = peak_count / 2.0
    lo_bin = peak_idx
    while lo_bin - 1 >= 0 and counts[lo_bin - 1] >= half:
        lo_bin -= 1
    hi_bin = peak_idx
    while hi_bin + 1 < n_bins and counts[hi_bin + 1] >= half:
        hi_bin += 1

    peak_lo_mm = float(edges[lo_bin])
    peak_hi_mm = float(edges[hi_bin + 1])

    # Refine sigma on PEAK SAMPLES ONLY - the key contrast vs the old method,
    # which measured noise on the contaminated full profile.
    in_peak = (y_mm >= peak_lo_mm) & (y_mm <= peak_hi_mm)
    peak_samples = y_mm[in_peak]
    if peak_samples.size < 5:
        meta["feature_rejection_reason"] = "peak_samples_insufficient"
        return [True] * n, meta

    mode_mm = float(np.median(peak_samples))
    peak_mad_mm = float(np.median(np.abs(peak_samples - mode_mm)))
    sigma_peak_mm = max(1.4826 * peak_mad_mm, 0.5 * bin_width_mm)

    # Bulk band: mode ± k * sigma_peak. k=5 keeps real material (including
    # modest conicity) and excludes the 5-15 um groove/hole walls.
    K_SIGMA = 5.0
    band_half_mm = K_SIGMA * sigma_peak_mm
    bulk_lo_mm = mode_mm - band_half_mm
    bulk_hi_mm = mode_mm + band_half_mm

    valid = (y_mm >= bulk_lo_mm) & (y_mm <= bulk_hi_mm)
    n_valid = int(np.sum(valid))
    n_removed = n - n_valid

    meta["feature_threshold_um"] = float(band_half_mm * 1000.0)

    if n_removed <= 0:
        meta["feature_rejection_reason"] = "no_samples_outside_bulk_band"
        return [True] * n, meta

    if n_valid < 6:
        meta["feature_rejection_reason"] = "too_few_valid_samples"
        return [True] * n, meta

    # feature_count is reported for the UI only; the filter itself is
    # location-independent.
    invalid_runs = _contiguous_true_runs((~valid).tolist())

    meta.update({
        "feature_count": len(invalid_runs),
        "feature_removed_samples": n_removed,
        "feature_rejection_used": True,
        "feature_rejection_reason": "ok",
    })
    return [bool(v) for v in valid.tolist()], meta


def _apply_mask(t_list, y_list, mask):
    return ([t for t, keep in zip(t_list, mask) if keep],
            [y for y, keep in zip(y_list, mask) if keep])
...
class ThicknessProcessor(object):
    ...
    def process_end_of_measurement(self):
        ...
        if self.state.valid_measurement_detected and self.state.sample_count > 0 and self.state.start_time is not None:
            ...
            # Build a mask for the bulk material. The full trimmed profile stays
            # available for debug, but statistics and UI use only valid points.
            valid_mask, feature_meta = _build_bulk_valid_mask(
                t_trim, y_trim, tol_used_um, Cfg.P2P_THRESH_UM
            )
            t_valid, y_valid = _apply_mask(t_trim, y_trim, valid_mask)
            ...
            # A through-hole / deep groove may have produced out-of-range samples
            # that were intentionally not included in the valid profile. In that
            # case the evidence is a time gap between the two bulk sides.
            gap_count, max_gap_sec = _count_internal_time_gaps(t_trim)
            if gap_count > 0 and not feature_meta.get("feature_rejection_used"):
                feature_meta.update({
                    "feature_count": gap_count,
                    "feature_removed_samples": 0,
                    "feature_rejection_used": True,
                    "feature_rejection_reason": "internal_time_gap",
                    "feature_max_gap_sec": max_gap_sec,
                })
            else:
                feature_meta.setdefault("feature_max_gap_sec", max_gap_sec)
            ...
            # Conicity by linear regression:
            # Fit a straight line to the averaged thickness profile.
            # x = physical axial position along the measured span [mm]
            # y = measured thickness [mm]
            if len(avg_y) >= 6:
                ...
                if conicity_span_mm > 1e-9:
                    ...
                    slope_mm_per_mm, intercept_mm = np.polyfit(x_mm, y_mm, 1)
            ...
            p = {
                ...
                "feature_removed_samples": int(feature_meta.get("feature_removed_samples", 0)),
                "feature_rejection_used": bool(feature_meta.get("feature_rejection_used", False)),
                "feature_rejection_reason": feature_meta.get("feature_rejection_reason", ""),
                "feature_threshold_um": feature_meta.get("feature_threshold_um", None),
                "feature_max_gap_sec": feature_meta.get("feature_max_gap_sec", None),
...
def render_thickness_panel_pil(state: ThicknessRuntimeState, last_n: int = 3, size_px=(420, 900)):
    ...
    plot = _profile_plot_pil(
        ...
        size_px=(W - 2 * pad, 250),
        feature_text=(
            (f"Feature removed: {payload.get('feature_removed_samples', 0)} samples"
             if int(payload.get('feature_removed_samples', 0) or 0) > 0
             else "Feature detected: internal gap")
            if payload.get("feature_rejection_used", False) else ""
        ),
    )
