...
(same PLC Modbus link, same sensor pipeline). Thickness now uses the module's
linearization calibration: thickness = a*(top+bottom) + b, with a,b taken from
the 'calibration' block of the config (via state.apply_calibration).
...
def build_readers(state):
    readers = []
    for name, info in state.cfg.SENSORS.items():
        readers.append(thck.SensorReader(state.cfg, state, name, info["port"]))
    return readers
...
def read_sensors(state):
    ...
    thr = state.cfg.ERROR_THRESHOLD_MM
    top_in = top is not None and top < thr
    bot_in = bot is not None and bot < thr
    # לינאריזציה: thickness = a*(top+bottom) + b (a,b מבלוק calibration בקונפיג).
    # apply_calibration מחזיר None כשאין כיול תקף (calibration.valid=false / חסרים a,b).
    thickness = state.apply_calibration(top + bot) if (top_in and bot_in) else None
...
def main():
    ...
    # sample buffers we read from here). Build the config explicitly, the same
    # way the module does: no module-global Cfg, and state/readers need cfg.
    cfg_path = os.path.join(os.path.dirname(thck.__file__),
                            "config", "config457_thk.json")
    cfg = thck.ThicknessConfig(thck.load_config_json(cfg_path))
    state = thck.ThicknessRuntimeState(cfg)
