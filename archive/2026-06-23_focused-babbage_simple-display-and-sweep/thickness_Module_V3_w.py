...
def _profile_plot_pil(profile_t, profile_th_mm, nominal_mm=None, tol_um=None, size_px=(420, 250)):
    ...
    # Use the actual canvas size: figsize*dpi may round down a pixel (e.g. 820 -> 819).
    actual_size = canvas.get_width_height()
    im = Image.frombuffer("RGBA", actual_size, buf, "raw", "RGBA", 0, 1)
    if actual_size != (w_px, h_px):
