...
# Thickness module singleton — created once by T3 and exposed at module level so the
# UI (and any other thread) can drive calibration through measure_gauge() without
# poking at T3's local state.
ThckModule = None
...
class Job_settings_():
    ...
    def __init__(self):
        ...
        # Thickness calibration view (written by T3 / button handlers, read by the UI).
        # Reassigned as a whole dict on each update so the cross-thread read is always
        # consistent. Keys match the two-gauge linearization model: thickness = slope*S+offset
        # where S1/S2 are the measured raw sums for the small/large reference gauges
        # whose true thicknesses are T1/T2.
        self.thk_cal = {
            "active": False, "phase": "idle",
            "T1_mm": None, "T2_mm": None,
            "S1_mm": None, "S1_std_um": None,
            "S2_mm": None, "S2_std_um": None,
            "slope": None, "offset_mm": None,
            "valid": False, "parts_since_cal": 0, "max_parts_before_recal": 0,
            "last_iso": None, "message": "",
        }
...
def _publish_cal_snapshot(**overrides):
    """Build a fresh thk_cal dict from the active module calibration + optional overrides
    and publish it atomically so any UI reader sees a consistent snapshot."""
    cal = {
        "active": False, "phase": "idle",
        "T1_mm": None, "T2_mm": None,
        "S1_mm": None, "S1_std_um": None,
    ...
    if ThckModule is not None:
        try:
            c = ThckModule.get_calibration()
            cal.update({
                "T1_mm": c.get("T1_mm"), "T2_mm": c.get("T2_mm"),
                "S1_mm": c.get("S1_mm"), "S2_mm": c.get("S2_mm"),
                "slope": c.get("slope"), "offset_mm": c.get("offset"),
                "valid": bool(c.get("valid")),
                "parts_since_cal": int(c.get("parts_since_cal") or 0),
                "max_parts_before_recal": int(c.get("max_parts_before_recal") or 0),
                "last_iso": c.get("last_calibration_iso"),
            })
        except Exception:
            pass
...
def _run_gauge_measurement(which: str):
    """Trigger a single-gauge measurement (blocking ~1s). Safe to call from a UI button
    via a background thread. Updates the thk_cal view as the sequence progresses."""
    if ThckModule is None:
        log("calibration: thickness module not started yet", "WARN")
        return
    phase = "measuring_small" if which == "small" else "measuring_large"
    _publish_cal_snapshot(active=True, phase=phase, message="")
    RegsToPlc[CAL_STAT_REG] = CAL_MEASURING
    log(f"calibration: measuring {which} gauge", "STATUS")

    res = ThckModule.measure_gauge(which)

    std_key = "S1_std_um" if which == "small" else "S2_std_um"
    overrides = {std_key: res.get("std_um")}

    if not res.get("ok"):
        RegsToPlc[CAL_STAT_REG] = CAL_FAIL
        _publish_cal_snapshot(active=True, phase="done_fail",
                              message=f"{which} gauge: {res.get('reason')}", **overrides)
        log(f"calibration: {which} gauge measurement failed ({res.get('reason')})", "WARN")
        return

    if res.get("calibration_applied"):
        RegsToPlc[CAL_STAT_REG] = CAL_OK
        _publish_cal_snapshot(active=True, phase="done_ok", message="", **overrides)
        log(f"calibration applied: slope={res.get('slope'):.4f} offset={float(res.get('offset')):+.4f} mm", "STATUS")
    else:
        # First gauge captured, awaiting the other one. CAL_G1_DONE remains the
        # "safe to move to the next gauge" semaphore the PLC waits on.
        RegsToPlc[CAL_STAT_REG] = CAL_G1_DONE
        next_phase = f"{which}_done"
        _publish_cal_snapshot(active=True, phase=next_phase,
                              message=res.get("reason", ""), **overrides)
        log(f"calibration: {which} gauge S={res.get('S_median_mm'):.4f}mm std={res.get('std_um'):.2f}um", "STATUS")
...
def T3_thickness():
    ...
    # Publish the initial calibration snapshot so the UI can immediately reflect
    # whether the module came up with a valid persisted calibration.
    _publish_cal_snapshot()
    ...
    while not Job_settings.stop_event.is_set():
        try:
                ...
                # PLC moves to the small gauge (cmd=1) or the large gauge (cmd=2) and
                # signals position. After both have been measured the module auto-fits
                # and applies the linearization, reporting OK/FAIL.
                cal_cmd = RegsFromPlc[CAL_CMD_REG]
                if cal_cmd != last_cal_cmd:
                    last_cal_cmd = cal_cmd
                    if cal_cmd in (1, 2):
                        which = "small" if cal_cmd == 1 else "large"
                        _run_gauge_measurement(which)
                    elif cal_cmd == 0:
                        RegsToPlc[CAL_STAT_REG] = 0  # back to idle (window keeps showing the last result)

                # Keep the calibration snapshot fresh so the UI can show parts-since-cal,
                # validity, and "recalibration required" without polling the module.
                _publish_cal_snapshot(active=Job_settings.thk_cal.get("active", False),
                                      phase=Job_settings.thk_cal.get("phase", "idle"),
                                      message=Job_settings.thk_cal.get("message", ""))
    ...
    def _open_thickness_adjustment_screen(self):
        ...
        def _thk_flat_keys():
            return [
                ...
                # --- Two-gauge linearization parameters (calibration block) ---
                ("gauge_small_known_mm (T1)", "calibration.gauge_small_known_mm"),
                ("gauge_large_known_mm (T2)", "calibration.gauge_large_known_mm"),
                ("calibration window_sec",    "calibration.window_sec"),
                ("stability_max_std_um",      "calibration.stability_max_std_um"),
                ("slope_min",                 "calibration.slope_min"),
                ("slope_max",                 "calibration.slope_max"),
                ("max_parts_before_recal",    "calibration.max_parts_before_recal"),
            ]
        ...
        def save_thk():
            ...
            for label, key_path in flat:
                ...
                if key_path == "schema_version":
                    continue
                if key_path in ("sensors.TOP.port", "sensors.BOTTOM.port", "max_error_count",
                                "target_points", "live_update_hz", "calibration.max_parts_before_recal"):
                    try:
                        val = int(raw) if raw else 0
                    except ValueError:
                        val = 0
                elif key_path in ("socket_timeout_sec", "frame_interval_sec", "time_sync_threshold_sec",
                                  "error_threshold_mm", "axial_span_mm", "thickness_min_mm", "thickness_max_mm",
                                  "trim_lo", "trim_hi", "p2p_thresh_um", "conicity_thresh_um",
                                  "nominal_thickness_mm_default", "tol_um_default",
                                  "calibration.gauge_small_known_mm", "calibration.gauge_large_known_mm",
                                  "calibration.window_sec", "calibration.stability_max_std_um",
                                  "calibration.slope_min", "calibration.slope_max"):
                    try:
...
#
    ...
    # ----------------------------
    # phase -> (display text, color). Driven by Job_settings.thk_cal["phase"].
    _CAL_PHASES = {
        "measuring_small": ("מודד מדיד קטן…", "#000000"),
        "measuring_large": ("מודד מדיד גדול…", "#000000"),
        "small_done":      ("מדיד קטן נמדד — ממתין למדיד הגדול", "#000000"),
        "large_done":      ("מדיד גדול נמדד — ממתין למדיד הקטן", "#000000"),
    ...
    def _build_thk_cal_window(self):
        ...
        # Validity banner — green when calibrated, red when not. The whole reason this
        # window exists is to drive that state, so it's the loudest element here.
        self._cal_valid_lbl = tk.Label(container, text="", font=("Arial", 13, "bold"),
                                       padx=10, pady=6, anchor="center")
        self._cal_valid_lbl.pack(fill="x", pady=(0, 8))

        # Phase line.
        self._cal_phase_lbl = ttk.Label(container, text="", font=("Arial", 14, "bold"))
        self._cal_phase_lbl.pack(anchor="center", pady=(0, 10))

        # Two big buttons — operator triggers a measurement once each reference gauge
        # is physically positioned between the sensors. Runs in a worker thread so the
        # UI never freezes during the ~1s sampling window.
        ...
        def _trigger(which):
            if ThckModule is None:
                return
            self._cal_small_btn.state(["disabled"])
            self._cal_large_btn.state(["disabled"])
            def _worker():
                try:
                    _run_gauge_measurement(which)
                finally:
                    try:
                        self.after(0, lambda: (self._cal_small_btn.state(["!disabled"]),
                                               self._cal_large_btn.state(["!disabled"])))
                    except Exception:
                        pass
            threading.Thread(target=_worker, daemon=True).start()

        self._cal_small_btn = ttk.Button(btns, text="מדוד מדיד קטן",
                                         command=lambda: _trigger("small"), padding=(10, 8))
        self._cal_small_btn.pack(side="left", expand=True, fill="x", padx=(0, 6))
        self._cal_large_btn = ttk.Button(btns, text="מדוד מדיד גדול",
                                         command=lambda: _trigger("large"), padding=(10, 8))
        self._cal_large_btn.pack(side="left", expand=True, fill="x", padx=(6, 0))

        grid = ttk.LabelFrame(container, text="Measurements", padding=10)
        grid.pack(fill="x")

        self._cal_small_var = tk.StringVar(value="—")
        self._cal_large_var = tk.StringVar(value="—")
        self._cal_known_var = tk.StringVar(value="—")
        self._cal_result_var = tk.StringVar(value="—")
        self._cal_age_var = tk.StringVar(value="—")

        def _row(parent, r, title, var):
            ttk.Label(parent, text=title, width=20, anchor="w").grid(row=r, column=0, sticky="w", pady=3)
            ttk.Label(parent, textvariable=var, font=("Consolas", 11)).grid(row=r, column=1, sticky="w", pady=3)

        _row(grid, 0, "Known T1 / T2:",    self._cal_known_var)
        _row(grid, 1, "Small gauge (S1):", self._cal_small_var)
        _row(grid, 2, "Large gauge (S2):", self._cal_large_var)
        _row(grid, 3, "Fit:",              self._cal_result_var)
        _row(grid, 4, "Parts since cal:",  self._cal_age_var)
        grid.grid_columnconfigure(1, weight=1)

        self._cal_msg_lbl = ttk.Label(container, text="", foreground="#444444", wraplength=480)
    ...
    def _open_view_calibration(self):
        """Operator-triggered: snapshot the current active calibration straight from the
        thickness module (which already loaded the persisted JSON on startup) and open
        the window. Subsequent measurements update the same window live."""
        if ThckModule is not None:
            try:
                _publish_cal_snapshot(active=False, phase="view", message="")
            except Exception:
                pass
    ...
    def _update_thk_cal_window(self):
        ...
        def _fmt_S(s_mm, std_um):
            if s_mm is None:
                return "—"
            s = f"S={float(s_mm):.4f} mm"
            if std_um is not None:
                s += f"   (±{float(std_um):.2f} µm)"
            return s

        # Validity banner — the operator's primary signal on whether the system will
        # accept measurements right now.
        valid = bool(cal.get("valid"))
        parts = int(cal.get("parts_since_cal") or 0)
        cap = int(cal.get("max_parts_before_recal") or 0)
        if valid:
            txt = f"כיול תקף ✓   ({parts}/{cap} חלקים)"
            self._cal_valid_lbl.config(text=txt, bg="#cfe9d6", fg="#0f5a1c")
        else:
            txt = "אין כיול תקף — המערכת לא תמדוד עד שיבוצע כיול דו-מדידי"
            self._cal_valid_lbl.config(text=txt, bg="#f4cccc", fg="#7a1414")
        ...
        T1 = cal.get("T1_mm"); T2 = cal.get("T2_mm")
        if T1 is not None and T2 is not None:
            self._cal_known_var.set(f"T1={float(T1):.4f} mm   T2={float(T2):.4f} mm")
        else:
            self._cal_known_var.set("— (set gauge_small_known_mm / gauge_large_known_mm in JSON)")

        self._cal_small_var.set(_fmt_S(cal.get("S1_mm"), cal.get("S1_std_um")))
        self._cal_large_var.set(_fmt_S(cal.get("S2_mm"), cal.get("S2_std_um")))

        slope = cal.get("slope"); offset = cal.get("offset_mm")
        if slope is not None and offset is not None:
            self._cal_result_var.set(f"slope={float(slope):+.4f}   offset={float(offset):+.4f} mm")
        else:
            self._cal_result_var.set("—")

        self._cal_age_var.set(f"{parts} / {cap}")
