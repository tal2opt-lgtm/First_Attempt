...
class ThicknessConfig(object):
    ...
    def __init__(self, cfg_dict: dict):
        ...
        # Sensor-reading threshold used to detect "no part between sensors" (empty gap).
        # This is the raw v_mm value above which a sensor is considered to see no part.
        ...
        # Thickness is defined exclusively by the linear map:
        #     thickness_mm = slope * (top_v_mm + bot_v_mm) + offset
        # where (slope, offset) are derived from two measurements taken on two physical
        # reference gauges whose true thickness (T1, T2) is known. Without a valid
        # two-gauge calibration the module produces NO thickness output at all.
        cal = d.get("calibration", {}) or {}
        # Known absolute thicknesses of the two reference gauges (configured, not measured).
        # T1 = small gauge, T2 = large gauge. Both are required for the module to operate.
        ...
        # Sanity bounds on the fitted slope. Physically slope ≈ -1 (sum decreases as
        # thickness grows), so default bounds are tight around -1.
        self.CAL_SLOPE_MIN = float(cal.get("slope_min", -1.10))
        self.CAL_SLOPE_MAX = float(cal.get("slope_max", -0.90))
        # Calibration validity ages with use: after this many measured parts a fresh
        # calibration is required before further measurements are accepted.
        self.CAL_MAX_PARTS_BEFORE_RECAL = int(cal.get("max_parts_before_recal", 100))
        # Persisted last-successful calibration (loaded on startup, written after every
        # successful 2-point fit). None means "not calibrated yet".
...
class ThicknessRuntimeState(object):
    def __init__(self, cfg: ThicknessConfig):
        ...
        # Loaded from JSON on startup; updated only by a successful 2-gauge measurement.
        ...
        self.parts_since_cal = 0               # incremented at every accepted part; resets on new calibration
        # Collection state used while a calibration gauge is sitting between the sensors.
        self.cal_collecting = False            # when True, run() routes S samples to cal_buffer instead of producing parts
        self.cal_buffer = []                   # raw S = top_v + bot_v samples gathered during a gauge measurement
    ...
    def is_calibration_valid(self):
        """A calibration is valid only when slope/offset were derived from a successful
        two-gauge fit AND the parts-since-calibration counter has not yet hit its limit."""
        with self.cal_lock:
    ...
    def invalidate_calibration(self):
        """Drop the active calibration (e.g., user demand or counter expiry)."""
        with self.cal_lock:
            self.slope = None
            self.offset = None
            ...
            while True:
                try:
                    ...
                    with self.state.lock:
                        ...
                        for v_mm in vals_mm:
                            last += dt
                            # Store the raw sensor reading directly. Any constant offset
                            # cancels through the two-gauge linearization downstream.
                            self.state.samples[self.name].append((last, v_mm))
...
class ThicknessProcessor(object):
    ...
    def run(self):
        while not self.state.stop_event.is_set():
            ...
            # Empty-gap detection: both sensors see "no part" → out-of-range raw readings.
            if top_v_mm >= self.cfg.ERROR_THRESHOLD_MM and bot_v_mm >= self.cfg.ERROR_THRESHOLD_MM:
            ...
            # The raw sum S = top + bot has NO physical meaning on its own. Thickness
            # exists only through the two-gauge linearization.
            S_mm = top_v_mm + bot_v_mm

            # Calibration capture mode: collect S samples for the gauge currently between
            # the sensors. No part/pass logic runs in this mode.
            if self.state.cal_collecting:
                self.state.cal_add_sample(S_mm)
                continue

            # Without a valid two-gauge calibration the module produces NOTHING. No part
            # logic, no UI live updates, no PLC verdict. Operator must (re)calibrate.
    ...
    def process_end_of_measurement(self):
        ...
        if self.state.valid_measurement_detected and self.state.sample_count > 0 and self.state.start_time is not None:
            ...
            # Age the calibration: one more measured part since last successful 2-gauge fit.
            # Once the limit is hit, is_calibration_valid() returns False and the processor
            # will stop accepting new parts until the operator runs calibration again.
...
class ThicknessModule(object):
    ...
    # =====================================================================
    # Calibration API — two precision reference gauges of known thickness.
    #
    # The two-gauge linearization is the ONLY source of absolute thickness in this
    # module. Until both gauges have been measured and a successful fit applied,
    # the processor produces no thickness output.
    #
    # Typical flow (driven by UI buttons or PLC handshake):
    #   1. Position small gauge between sensors.
    #   2. measure_gauge("small")  → records S1 and (if also S2 is present) computes the fit.
    #   3. Position large gauge between sensors.
    #   4. measure_gauge("large")  → records S2 and (with S1 already present) computes the fit.
    # =====================================================================
    def measure_gauge(self, which: str, window_sec: float | None = None):
        """Measure the raw sum S = top_v + bot_v for a reference gauge currently sitting
        between the sensors. `which` must be 'small' (T1) or 'large' (T2).

        Collects S samples for `window_sec` (default from JSON), checks stability,
        stores the median in state, and — if BOTH gauges are now present — automatically
        runs the two-point fit and applies it.

        Returns dict: {ok, which, reason, n, S_median_mm, std_um, calibration_applied,
                       slope, offset}.
        """
        ...
        if std_um > float(self.cfg.CAL_STABILITY_MAX_STD_UM):
            out["reason"] = f"unstable: std {std_um:.2f}µm > {self.cfg.CAL_STABILITY_MAX_STD_UM:.2f}µm"
            return out

        # Stage the measurement on state and attempt the fit if both gauges are present.
        with self.state.cal_lock:
            if which == "small":
                self.state.S1_mm = S_med
            else:
                self.state.S2_mm = S_med
            S1, S2 = self.state.S1_mm, self.state.S2_mm

        out["ok"] = True

        if S1 is not None and S2 is not None:
            ok_fit, fit_info = self._fit_and_apply_calibration(S1, S2)
    ...
    def _fit_and_apply_calibration(self, S1_mm, S2_mm):
        """Two-point fit: thickness_mm = slope * S + offset, where
            slope  = (T2 - T1) / (S2 - S1)
            offset = T1 - slope * S1
        Validates slope sign/magnitude, applies live, and persists to JSON.
        """
        info = {"slope": None, "offset": None, "reason": ""}
        T1, T2 = self.cfg.CAL_T1_MM, self.cfg.CAL_T2_MM
        if T1 is None or T2 is None:
            info["reason"] = "known gauge thicknesses (gauge_small_known_mm / gauge_large_known_mm) not configured"
            return False, info
        ...
        if not (self.cfg.CAL_SLOPE_MIN <= slope <= self.cfg.CAL_SLOPE_MAX):
            info["reason"] = (f"slope {slope:.4f} out of bounds "
                              f"[{self.cfg.CAL_SLOPE_MIN}, {self.cfg.CAL_SLOPE_MAX}]")
            return False, info

        iso = time.strftime("%Y-%m-%dT%H:%M:%S")
        self.state.set_calibration(slope, offset, S1_mm, S2_mm, iso)
        self.cfg.CAL_SLOPE = slope
        self.cfg.CAL_OFFSET = offset
        self.cfg.CAL_S1_MM = float(S1_mm)
        self.cfg.CAL_S2_MM = float(S2_mm)
        self.cfg.CAL_LAST_ISO = iso
        self._save_calibration(slope, offset, S1_mm, S2_mm, iso)
        info["reason"] = "ok"
        return True, info

    def invalidate_calibration(self, reason: str = ""):
        """Force the module back into 'not calibrated' mode. The processor will then
        block all thickness output until a fresh two-gauge calibration is performed."""
        self.state.invalidate_calibration()

    def get_calibration(self):
        """Snapshot of the active calibration."""
        with self.state.cal_lock:
            return {
                "valid": self.state.is_calibration_valid() if (self.state.slope is not None and self.state.offset is not None) else False,
                "T1_mm": (float(self.cfg.CAL_T1_MM) if self.cfg.CAL_T1_MM is not None else None),
                "T2_mm": (float(self.cfg.CAL_T2_MM) if self.cfg.CAL_T2_MM is not None else None),
                "S1_mm": (float(self.state.S1_mm) if self.state.S1_mm is not None else None),
                "S2_mm": (float(self.state.S2_mm) if self.state.S2_mm is not None else None),
                "slope": (float(self.state.slope) if self.state.slope is not None else None),
                "offset": (float(self.state.offset) if self.state.offset is not None else None),
                "last_calibration_iso": self.state.cal_last_iso,
                "parts_since_cal": int(self.state.parts_since_cal),
                "max_parts_before_recal": int(self.cfg.CAL_MAX_PARTS_BEFORE_RECAL),
            }

    def is_calibration_valid(self) -> bool:
        return bool(self.state.is_calibration_valid())

    def _save_calibration(self, slope, offset, S1_mm, S2_mm, iso):
        """Persist the calibration results into the 'calibration' block of the config JSON,
        ...
        try:
            ...
            # T1/T2 are configuration (set by the engineer), but we mirror them here as
            # an unambiguous record of what was used for THIS calibration.
            if self.cfg.CAL_T1_MM is not None:
                cal["gauge_small_known_mm"] = float(self.cfg.CAL_T1_MM)
            if self.cfg.CAL_T2_MM is not None:
                cal["gauge_large_known_mm"] = float(self.cfg.CAL_T2_MM)
