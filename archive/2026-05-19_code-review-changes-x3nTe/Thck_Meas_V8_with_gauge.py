        ...
        while not self.state.stop_event.is_set():
            while True:
                try:
                    ...
                    dt = self.cfg.FRAME_INTERVAL_SEC / num
                    t_base = t_packet - self.cfg.FRAME_INTERVAL_SEC

                    with self.state.lock:
                        for i, v_mm in enumerate(vals_mm):
                            t_samp = t_base + i * dt
                            if t_samp <= self.state.last_ts[self.name]:
                                t_samp = self.state.last_ts[self.name] + 1e-6
                            self.state.last_ts[self.name] = t_samp

                            abs_mm = self.cfg.REFERENCE_HEIGHT_MM + v_mm
                            self.state.samples[self.name].append((t_samp, abs_mm))
...
class ThicknessProcessor(object):
    ...
    def run(self):
        while not self.state.stop_event.is_set():
            ...
            # This handles both end-of-part (sensors empty -> thickness very large/small)
            ...
            thickness = self.cfg.SENSOR_DISTANCE_MM - (top_abs_mm + bot_abs_mm)
...
# >>> NEW: Update RAW stats cards (under plots)
        ...
        if mode == "RAW (single)":
            ...
            # Average the AVG-traces over the last up to 3 completed passes (including current)
            ...
            if last3:
                ...
                for pp in last3:
                    ...
                    if len(tt) and len(yy):
                        if len(tt) == len(t_common) and np.allclose(tt, t_common):
                            ys.append(yy)
                        else:
                ...
                if ys:
                    ...
                    _plot_avg(
                        ...
                        err3,
                        tol,
                        "AVG (last 3 passes) — error vs nominal",
                    )
                    shown_err = err3
                else:
                    ax_main.clear()
