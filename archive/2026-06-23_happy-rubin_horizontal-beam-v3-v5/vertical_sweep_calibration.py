...
HST_IN_POS6  = 6        # M6 status: in POS6
VCMD_STOP    = 7        # M7: STOP (kept only for the emergency-stop safety net)
VCMD_STEP_DOWN = 4      # M7: GoTo "POS4" = HMI-configured RELATIVE step down,
                        # PLC handles the precise distance and stops itself.
                        # This is the ONLY M7 motion command we send: no JOG.


# ----- Sweep parameters -------------------------------------------------------
# JOG_MM is the step distance configured on the HMI for POS4. Used only as a
# nominal X-axis label (position_mm = step * JOG_MM); the PLC is what actually
# enforces the distance, so set this to match whatever the HMI is set to.
JOG_MM = 0.50

SETTLE_SEC     = 0.8    # wait for the axis to fully stop + the sensor stream
                        # to refill before each measurement. Sized to cover the
                        # POS4 move time + ramp + sensor stream warmup.
...
M6_TIMEOUT_SEC      = 30.0
M6_STABLE_TOL       = 50
M6_STABLE_DWELL_SEC = 1.5

MAX_STEPS      = 200    # safety cap on POS4 steps
LIN_TOL_UM     = 4.0    # max thickness deviation from segment median [um]
LIN_MIN_POINTS = 5
...
# =============================================================================
class Plc:
    """Modbus link and M6/M7 motion. Every write shares one 20-register frame
    ...
    def step_down(self):
        """Trigger POS4: PLC moves M7 the HMI-configured relative distance
        downward and stops itself. Then wait SETTLE_SEC for the motion to
        finish and the sensor stream to refill."""
        self._write(reg10=VCMD_STEP_DOWN)
...
def read_position(module, cfg):
    ...
    Flush, wait for fresh frames, return the median of the newest SAMPLE_N
    samples per sensor. A sensor with too few fresh frames is reported as
    None (distinct from out-of-range)."""
    flush(module)
...
# =============================================================================
# Sweep:  POS4 step-down only - no JOG anywhere
# =============================================================================
def run_sweep(module, plc):
    """Precondition: M7 is positioned ABOVE the gauge band (both sensors
    out of range). The script then sends POS4 once per step, the PLC moves
    the HMI-configured distance down and stops itself, and we record. Done
    when the band has been crossed (both sensors out again at the bottom).

    No JOG: the only M7 motion command we send is reg10=4 (POS4 trigger)."""
    cfg = module.cfg
    rows = []
    ...
    # Precondition check: M7 must be above the band (both sensors out of range).
    # If a sensor sees the gauge we cannot proceed - we have no JOG up to back
    # off with, so the operator has to position manually first.
    r = read_position(module, cfg)
    if r["top_in_range"] or r["bottom_in_range"]:
        raise RuntimeError(
            "M7 is NOT above the gauge band - cannot start the sweep.\n"
            "  Move M7 UP manually (from the HMI) until BOTH sensors read\n"
            "  out-of-range, then run the script again.\n"
            f"  Current TOP    = {r['top_mm']}  (in_range={r['top_in_range']})\n"
            f"  Current BOTTOM = {r['bottom_mm']}  (in_range={r['bottom_in_range']})"
        )

    print("[start] above the band (both sensors out). Stepping down with POS4...")
    record(0, r)
    ...
    for s in range(1, MAX_STEPS + 1):
        ...
        if r["top_in_range"] and r["bottom_in_range"]:
            been_in_range = True
        # Done once we've been through both-in-range and now lost it again.
        if been_in_range and not (r["top_in_range"] and r["bottom_in_range"]):
            print(f"[sweep] band fully crossed after {s} steps")
            break
    else:
        print("[sweep] WARNING: hit MAX_STEPS before exiting the band")
...
def export_plot(rows, summary, path):
    ...
    labels every 5 um, dotted minor lines every 1 um."""
    try:
        import matplotlib
        matplotlib.use("Agg")
...
def main():
    ...
    # SAFETY: POS4 moves are self-stopping at the PLC, so a crash mid-sweep
    # can't cause a runaway by itself. We still register STOP on every exit
    # path as a belt-and-suspenders for any out-of-band command that might
    # be in flight.
    ...
    try:
        ...
        # Precondition for the POS4-only flow: M7 must be ABOVE the gauge band
        # (both sensors out of range). run_sweep() enforces this and raises a
        # clear error if not - we don't pre-empt it here so the operator sees
        # the same single point of truth.
