# -*- coding: utf-8 -*-
"""
Long-duration thickness DRIFT logger.
=====================================
...
def _make_plot(rows, png_path, nominal_mm, alert_um):
    ...
    try:
        ...
        if have_sensors:
            ...
            ax2.plot(s_times, top_drift, "-", color="#2ca02c", linewidth=1.0, label="TOP sensor drift")
            ax2.plot(s_times, bot_drift, "-", color="#ff7f0e", linewidth=1.0, label="BOTTOM sensor drift")
            ...
            while deadline is None or time.time() < deadline:
                ...
                if thickness_mm is None:
                    ...
                    print(f"[{now:%H:%M:%S}] {thickness_mm:.5f} mm  "
                          f"dev {deviation_um:+.2f} um  std {std_um:.2f} um  n={n}{flag}")
                if alert:
                    _beep()
